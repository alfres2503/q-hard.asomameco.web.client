...
const EditAssociate = () => {
  ...
  const handleSubmit = async (values: any) => {
    try {
      setIsLoading(true);
      const response = await GenericService.update("associates", values);
      ...
      Notification("Asociado actualizado con éxito");
  ...
  const handleChangeState = async (values: any) => {
    setAssociate({ ...associate!, isActive: !associate?.isActive, id: associate?.id || 0 });
  };
    ...
    <Layout>
      <section className="p-0 md:p-0  max-w-9xl flex flex-col justify-center gap-5 mx-10 mt-10">
        <div className="flex justify-start items-center text-center gap-3 text-xl font-bold">
          ...
          <h1 className="">Editar un Asociado</h1>
        </div>

        <Formik
          initialValues={{
            id: associate?.id,
            idCard: associate?.idCard,
            name: associate?.name,
            ...
            <Form>
              <div className="w-full px-4 lg:px-0 mx-auto my-2 gap-5">
                ...
                <div className="mt-8 flex gap-3 items-center justify-between">
                  ...
                  >
                    {isLoading ? "Cargando..." : "Editar Asociado"}
                  </Button>
                </div>
              </div>
            </Form>
