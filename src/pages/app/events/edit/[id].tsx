...
const EditEvent = () => {
  ...
  const handleSubmit = async (values: any) => {
    try {
     ...
     if(values.time.toString().length < 5){
      ...
      const response = await GenericService.update("events", values);
      ...
      Notification("Evento actualizado con éxito");
  ...
  const handleChangeMember = async (values: any) => {
    setEvent({ ..._event!, idMember: Number(values) });
  };
  const handleChangeCaterine = async (values: any) => {
    setEvent({ ..._event!, idCateringService: Number(values) });
  };
    ...
    <Layout>
      <section className="p-0 md:p-0  max-w-9xl flex flex-col justify-center gap-5 mx-10 mt-10">
        <div className="flex justify-start items-center text-center gap-3 text-xl font-bold">
          ...
          <h1 className="">Editar evento</h1>
        </div>

        <Formik
          initialValues={{
            id: _event?.id,
            idMember: _event?.idMember,
            name: _event?.name,
            description: _event?.description,
            date: _event?.date,
            time: _event?.time,
            place: _event?.place,
            idCateringService: _event?.idCateringService,
          }}
            ...
            <Form>
              <div className="pb-1 pt-1 flex items-center rounded-lg border-2 border-gray-300 mb-3">
                ...
                <Select
                  ...
                  name="idMember"
                  placeholder="Miembro"
                  value={_event?.idMember.toString() || ""}
              ...
              <div className="w-full px-4 lg:px-0 mx-auto my-2 gap-5">
                ...
                <div className="pb-1 pt-1 flex items-center rounded-lg border-2 border-gray-300 mb-3">
                  ...
                  <Select
                    ...
                    value={_event?.idCateringService.toString() || ""}
